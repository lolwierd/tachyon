  ...
  const requestPromise = (async () => {
    ...
    for (let attempt = 0; attempt <= MAX_RETRIES; attempt += 1) {
      try {
        ...
        lastError = error instanceof Error ? error : new Error("Unknown TheBlank fetch error");

        if (isRetryableError(lastError) && attempt < MAX_RETRIES) {
          logWarn("source.theblank.retry", {
            url,
            attempt: attempt + 1,
    ...
    const finalError = lastError ?? new Error("Unknown TheBlank fetch error");
    logError("source.theblank.request_failed", finalError, {
      url,
      method: options?.method || "GET",
    });
    throw finalError;
...
async function fetchWithThrottle(
  ...
  if (!res.ok) {
    throw new Error(
      `TheBlank request failed: ${res.status} ${res.statusText} — ${url}`,
    );
  }
...
function parseChapterSlug(href: string): string {
  // /manga/{slug}/{chapter-slug}/ → last path segment
...
export async function getSeriesDetail(
  ...
  $(".related-manga a[href*='/manga/'], .manga-related a[href*='/manga/']").each((_, a) => {
...
export async function getChapterList(
  ...
  const url = `${BASE_URL}/manga/${sourceId}/ajax/chapters/`;
  const html = await throttledFetch(url, {
    method: "POST",
...
export async function getChapterPages(
  ...
  const url = `${BASE_URL}/manga/${chapterSourceId}/`;
  const html = await throttledFetch(url, {
    referer: `${BASE_URL}/`,
  });
  const $ = cheerio.load(html);
...
function getChapterUrl(chapterSourceId: string) {
  return `${BASE_URL}/manga/${chapterSourceId}/?style=list`;
}

registerSource({
  name: "theblank",
  displayName: "The Blank",
  baseUrl: BASE_URL,
  isNsfw: true,
